    ...
    <Shell>
       <div className="flex flex-col gap-10">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
             <div>
                <h1 className="text-3xl font-bold font-display text-foreground mb-2">My Classrooms</h1>
                <p className="text-muted-foreground">Pick up where you left off.</p>
             </div>
             <Button className="gap-2 bg-secondary text-foreground hover:bg-white/20 border-border backdrop-blur-md" variant="outline">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             {MY_COURSES.map(course => (
                <Link key={course.id} href="/course/1">
                   <div className="group relative overflow-hidden rounded-2xl bg-card border border-border hover:border-primary/50 transition-all cursor-pointer">
                      <div className="flex h-40 md:h-48">
                         ...
                         <div className="flex-1 p-6 flex flex-col justify-between bg-gradient-to-r from-card to-transparent">
                            <div>
                               <h3 className="text-xl font-bold font-display text-foreground mb-1 group-hover:text-primary transition-colors">{course.title}</h3>
          ...
          <div className="space-y-6">
             <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold font-display text-foreground">Explore Topics</h2>
             ...
             {isLoading ? (
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                 {coursesList.map(course => (
                   <div key={course.id} className="group flex flex-col rounded-xl overflow-hidden bg-secondary border border-border hover:border-border transition-all" data-testid={`course-${course.id}`}>
                     <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-primary/40 via-card to-accent/30 flex items-center justify-center">
                       {course.thumbnail ? (
                         ...
                         <BookOpen className="h-12 w-12 text-muted-foreground group-hover:text-foreground/60 transition-colors" />
                       )}
                       {course.level && (
                         <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md px-2 py-1 rounded text-xs font-bold text-white border border-border">
                           {course.level}
                         </div>
                       )}
                     </div>
                     <div className="p-4 flex-1 flex flex-col">
                       <h3 className="font-bold text-foreground text-lg mb-1 group-hover:text-primary transition-colors line-clamp-1">{course.title}</h3>
