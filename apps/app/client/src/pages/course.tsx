...
function CurriculumList() {
    return (
        <div className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col h-full lg:h-fit lg:sticky lg:top-6">
           <div className="p-6 border-b border-border bg-secondary">
              <h3 className="font-bold font-display text-foreground text-lg mb-2">Course Content</h3>
              <div className="flex items-center justify-between text-sm mb-2">
                 <span className="text-muted-foreground">35% Completed</span>
                 <span className="text-foreground font-mono">3/8</span>
              </div>
           ...
           <div className="overflow-y-auto max-h-[calc(100vh-200px)] lg:max-h-[600px]">
              {MODULES.map((module) => (
                 <div key={module.id} className="border-b border-border last:border-0">
                    <div className="bg-secondary px-6 py-3 text-sm font-bold text-foreground/80 sticky top-0 backdrop-blur-sm z-10">
                       {module.title}
                    </div>
                    <div>
                       {module.lessons.map((lesson) => (
                          <div 
                             key={lesson.id} 
                             className={cn(
                                "flex items-center gap-3 px-6 py-4 hover:bg-secondary transition-colors cursor-pointer border-l-2",
                          ...
                          >
                             ...
                             <div className="flex-1 min-w-0">
                                <p className={cn("text-sm font-medium truncate", lesson.current ? "text-primary" : "text-foreground")}>
                                   {lesson.title}
                                </p>
    ...
    <Shell>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Video Player */}
          <div className="relative aspect-video rounded-2xl overflow-hidden bg-gradient-to-br from-purple-900 via-blue-900 to-cyan-900 border border-border shadow-2xl">
             ...
             <div className="absolute bottom-0 left-0 right-0 p-6 z-20">
                <h2 className="text-xl md:text-2xl font-bold font-display text-foreground mb-2 line-clamp-1">The Three Point Lighting Setup</h2>
          ...
          <div className="lg:hidden">
              <Sheet>
                  <SheetTrigger asChild>
                      <Button className="w-full bg-secondary hover:bg-white/20 text-foreground border border-border" variant="outline">
                          <BookOpen className="mr-2 h-4 w-4" /> View Curriculum
                      </Button>
                  </SheetTrigger>
                  <SheetContent side="bottom" className="h-[80vh] p-0 bg-card border-t border-border">
                      <CurriculumList />
                  </SheetContent>
              </Sheet>
          </div>
          ...
          <Tabs defaultValue="overview">
             ...
             <TabsContent value="overview" className="space-y-6 mt-6">
                <div className="flex items-start justify-between">
                   <div>
                      <h1 className="text-3xl font-bold font-display text-foreground mb-2">Cinematography Masterclass 101</h1>
                      <div className="flex items-center gap-2">
                         <Avatar className="h-8 w-8">
                            <AvatarImage src="https://api.dicebear.com/7.x/avataaars/svg?seed=mikeastro" />
                            <AvatarFallback>AM</AvatarFallback>
                         </Avatar>
                         <span className="text-muted-foreground">Instructor: <span className="text-foreground font-medium">Mike Astro</span></span>
                      </div>
                   </div>
                   <Button variant="outline" className="border-border">Share</Button>
                </div>
             ...
             <TabsContent value="discussion">
                <div className="p-8 text-center text-muted-foreground bg-secondary rounded-xl border border-border">
                   <MessageSquare className="mx-auto h-10 w-10 mb-4 opacity-50" />
                   <h3 className="text-lg font-bold text-foreground">Class Discussion</h3>
