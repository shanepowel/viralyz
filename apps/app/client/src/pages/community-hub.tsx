...
function ChannelsSidebar({ activeChannel, setActiveChannel }: { activeChannel: string, setActiveChannel: (id: string) => void }) {
  return (
    <div className="flex flex-col h-full bg-secondary/40">
       {/* Server Header */}
       <div className="p-4 border-b border-border hover:bg-secondary transition-colors cursor-pointer flex items-center justify-between group h-14">
          <h2 className="font-bold text-foreground font-display">Neon City</h2>
          ...
          <div className="space-y-6">
             <div>
                <h3 className="px-2 text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 flex items-center justify-between">
                   Text Channels <Plus size={12} className="cursor-pointer hover:text-foreground" />
                </h3>
                   ...
                   {CHANNELS.filter(c => c.type === 'text').map(channel => (
                      <div 
                         ...
                         className={cn(
                            ...
                            activeChannel === channel.id ? "bg-secondary text-foreground" : "text-muted-foreground hover:bg-secondary hover:text-foreground"
                         )}
                      >
             ...
             <div>
                <h3 className="px-2 text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 flex items-center justify-between">
                   Voice Channels <Plus size={12} className="cursor-pointer hover:text-foreground" />
                </h3>
                <div className="space-y-1">
                   {CHANNELS.filter(c => c.type === 'voice').map(channel => (
                      <div key={channel.id} className="group">
                         <div className="flex items-center gap-2 px-2 py-1.5 rounded-md cursor-pointer text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors">
       ...
       {/* User Status Footer */}
       <div className="p-3 bg-muted/60 flex items-center gap-3 border-t border-border">
          ...
          <div className="flex-1 min-w-0">
             <div className="text-xs font-bold text-foreground truncate">NeonDreamer</div>
...
export default function CommunityHub() {
  ...
  return (
    <Shell>
       <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 h-[calc(100vh-140px)] border border-border rounded-2xl overflow-hidden bg-card">
          
          {/* Desktop Channels Sidebar */}
          <div className="hidden lg:flex lg:col-span-3 flex-col border-r border-border">
          ...
          <div className="lg:col-span-9 flex flex-col bg-background/50">
             ...
             <div className="h-14 border-b border-border flex items-center justify-between px-4 bg-secondary">
                <div className="flex items-center gap-2">
                   ...
                   <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
                      ...
                      <SheetContent side="left" className="p-0 border-r border-border bg-card w-[85vw] sm:w-[350px]">
                   ...
                   <h3 className="font-bold text-foreground">{activeChannel}</h3>
                   <span className="text-xs text-muted-foreground border-l border-border pl-2 ml-2 hidden sm:block">Topic of discussion goes here...</span>
                </div>
                <div className="flex items-center gap-4">
                   <Bell size={18} className="text-muted-foreground hover:text-foreground cursor-pointer" />
                   <Users size={18} className="text-muted-foreground hover:text-foreground cursor-pointer" />
                   <div className="relative w-48 hidden md:block">
                      <input 
                         type="text" 
                         placeholder="Search" 
                         className="w-full bg-background border border-border rounded text-xs px-2 py-1 text-foreground placeholder:text-muted-foreground focus:outline-hidden" 
                      />
                   </div>
                </div>
             </div>
             ...
             <ScrollArea className="flex-1 p-4">
                <div className="space-y-6">
                   ...
                   {MESSAGES.map((msg) => (
                      <div key={msg.id} className="flex gap-4 group hover:bg-secondary p-2 -mx-2 rounded-lg transition-colors">
                         ...
                         <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                               <span className="font-bold text-foreground cursor-pointer hover:underline">{msg.user}</span>
             ...
             {/* Input Area */}
             <div className="p-4 bg-secondary mx-4 mb-4 rounded-xl border border-border">
                <div className="flex items-center gap-2">
                   <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-foreground rounded-full bg-secondary">
                      <Plus size={16} />
                   </Button>
                   <input 
                      type="text" 
                      placeholder={`Message #${activeChannel}`}
                      className="flex-1 bg-transparent border-none text-sm text-foreground placeholder:text-muted-foreground focus:outline-hidden"
                   />
